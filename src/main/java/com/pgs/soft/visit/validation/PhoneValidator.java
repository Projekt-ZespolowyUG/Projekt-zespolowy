...
public class PhoneValidator implements ConstraintValidator<Phone, String> {
    ...
    public boolean isValid(String phoneNo, ConstraintValidatorContext ctx) {
        ...
        if (phoneNo.matches("^(?=.{2,12}$)\\d[0-9]*")) return true;
     ...
     //   else if(phoneNo.matches("\\d{3}[-\\.\\s]\\d{2}[-\\.\\s]\\d{2}")) return true;
     //   else if(phoneNo.matches("\\d{3}-\\d{2}-\\d{2}\\s(x|(ext))\\d{3,5}")) return true;
